...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!payload || !Array.isArray(payload.matches)) return res.status(400).json({ error: 'Invalid payload' });
    ...
    const { data, error } = await supabase.from('matches').insert(rows).select('id, team_a_id, team_b_id');
