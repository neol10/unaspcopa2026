...
const LiveMatchControl: React.FC<{ match: Match }> = ({ match }) => {
  ...
  return (
    <div className="live-event-panel-wrapper">
      ...
      {match.id === 'e4b7e6d5-59d4-48e0-9209-460bf787cd23' && (
        <div style={{ padding: '1rem', background: '#FF0055', color: '#FFF', borderRadius: '8px', marginBottom: '1rem', textAlign: 'center' }}>
          <h3>⚠️ Correção Automática (ARG x HOL)</h3>
          <p>Clique no botão abaixo para apagar os lances atuais e registrar todos os gols/assistências corretos desta partida, como você pediu.</p>
          <button className="btn-save" style={{ background: '#FFF', color: '#FF0055', fontWeight: 'bold', marginTop: '10px' }} onClick={async () => {
            try {
              const matchId = match.id;
              const rick = '017cb776-3e4f-405a-9962-dd2bbd6cc6e8';
              const lucasAlmeida = '87fe2a86-33a2-4050-802b-c7c08e13e955';
              ...
              refreshEvents();
              alert('✅ LANCES CORRIGIDOS COM SUCESSO! \nDesça a tela e clique no botão RECONTAR ESTATÍSTICAS para recalcular tudo no ranking geral.');
            } catch (err: any) {
              alert('Erro: ' + err.message);
            }
          }}>
            Executar Correção (Apagar atuais e refazer)
          </button>
        </div>
      )}
      ...
      >
        ...
        <div className="lane">
          ...
          <div className="roster-section">
            {!isPreGame ? (
              /* Durante a partida: Exibir TODOS os jogadores em uma única lista limpa e ordenada */
              <div className="roster-group">
                <div className="roster-group-header">
                  <span className="roster-group-title">🏃 Jogadores ({playersA.length})</span>
                </div>
                <div className="admin-player-btns">
                  {(playersA || []).slice().sort((a,b) => (Number(a.number) || 0) - (Number(b.number) || 0)).map(p => {
                    ...
                    const isTitular = onFieldA.includes(p.id);
                    ...
                    return (
                      <button 
                        ...
                        className={`p-btn ${isTitular ? 'active-field' : 'bench'} event-${eventType}`}
                      >
                        <span className="p-num">{p.number}</span>
              ...
              /* Pré-jogo: Escalação de Titulares vs Reservas */
              <>
                <div className="roster-group">
                  <div className="roster-group-header">
        ...
        <div className="lane">
          ...
          <div className="roster-section">
            {!isPreGame ? (
              ...
              <div className="roster-group">
                <div className="roster-group-header">
                  <span className="roster-group-title">🏃 Jogadores ({playersB.length})</span>
                </div>
                <div className="admin-player-btns">
                  {(playersB || []).slice().sort((a,b) => (Number(a.number) || 0) - (Number(b.number) || 0)).map(p => {
                    const matchEvents = events || [];
                    const yellows = matchEvents.filter(e => e.player_id === p.id && e.event_type === 'amarelo').length;
                    const reds = matchEvents.filter(e => e.player_id === p.id && e.event_type === 'vermelho').length;
                    const isTitular = onFieldB.includes(p.id);
